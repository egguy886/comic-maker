...
================= 功能模块 =================
def get_api_key(): """尝试从服务器机密或侧边栏获取 API Key""" if "GOOGLE_API_KEY" in st.secrets: return st.secrets["GOOGLE_API_KEY"] else: return st.sidebar.text_input("请输入 Google API Key", type="password")

def generate_script(api_key, story_idea): """模块1：让 Gemini 写剧本""" try: genai.configure(api_key=api_key) # 使用 Gemini 1.5 Pro (或者 Flash) model = genai.GenerativeModel("gemini-1.5-pro-latest", system_instruction=SYSTEM_PROMPT) response = model.generate_content(f"Create a comic script for: {story_idea}") return response.text except Exception as e: st.error(f"剧本生成出错: {e}") return None
...
response = imagen_model.generate_images(
    ...
    st.error(f"绘图失败 (请检查你的 API Key 是否支持 Imagen 权限): {e}")
    return None
